...
(function ($) {
    ...
    $(document).on('click', '.submit', function () {
        var error = false,
            ...
            telFormat = /[0-9 -()+]+$/,
            actionURL = formObj.attr('action') || formObj.prop('action') || window.location.href,
            resultsObj = formObj.find('.form-results'),
            grecaptchav3 = _this.attr('data-sitekey') || '',
            antiForgeryToken = formObj.find('input[name="__RequestVerificationToken"]').val(),
        ...
        // Google reCaptcha verify (v2 widget handled here; v3 token injected before AJAX)
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (redirectVal != '' && redirectVal != undefined) {
                        ...
                        if (formObj.find('.g-recaptcha').length) { grecaptcha.reset(); }
                        resultsObj.removeClass('alert-success').removeClass('alert-danger').hide();
                        if (result && result.alert) { resultsObj.addClass(result.alert).html(result.message || ''); }
                        resultsObj.removeClass('d-none').fadeIn('slow').delay(4000).fadeOut('slow');
                    }
                },
                error: function () {
                    _this.removeClass('loading');
                    resultsObj.removeClass('alert-success').addClass('alert-danger').html('Došlo je do pogreške. Pokušajte ponovno.').removeClass('d-none').fadeIn('slow').delay(4000).fadeOut('slow');
                }
            });
        }
        ...
        if (!error && actionURL != '' && actionURL != undefined) {
            ...
            // If v3 site key is present, get token first
            if (typeof (grecaptcha) !== 'undefined' && grecaptcha !== null && grecaptchav3 != '' && grecaptchav3 != undefined) {
                grecaptcha.ready(function () {
                    grecaptcha.execute(grecaptchav3, { action: 'submit' }).then(function (token) {
                        formObj.find('input[name="g-recaptcha-response"]').val(token);
                        doAjaxSubmit();
                    });
                });
            } else {
